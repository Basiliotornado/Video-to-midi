...
parser.add_argument("--bpm",
                    ...
                    default=120)
parser.add_argument("--ascii-res",
                    type=str,
                    help="Resolution of output ascii. default=60,60",
                    default='60,60')
...
ppqn = args.ppqn
bpm = args.bpm
ascii_size = tuple(int(i) for i in args.ascii_res.split(','))
video_res = tuple(int(i) for i in args.video_res.split(',')) #Hate this but sue me
...
if __name__ == "__main__":
    ...
    frames, frame_count, frame_rate = AsciiImage.ascii_video(file, ascii_size, video_res)
    ...
    while sum(done) < note_count:
        ...
        time_until_video -= time
        if time_until_video <= 0:
            if video_frame >= frame_count:
                print('out of frames for video!')
            else:
                midi.tracks[0].append(mido.MetaMessage('lyrics', text=frames[video_frame]))
                time_until_video = video_interval + time_until_video
                video_frame += 1
